...
def _lambda_handler(event, context):
    ...
    # An empty _bulk body is rejected by OpenSearch, so skip batches with nothing to send
    if not opensearch_actions:
        logger.info('No OpenSearch actions in batch')
        return
...
# Global lambda handler - logs and re-raises so the event source mapping's
# bounded retries, batch bisection and on-failure queue handle the batch
