...
- **Accounts.** `dev`, `pre-production` and feature environments deploy to the current development account in us-east-1. `production` is in a separate account. The account ID is passed with `-c account=<id>` rather than stored in the repo. Production is not touched in this work.
...
- **Sizing.** Chosen by the `-c production` flag (default false), not the environment name. Without it: 1 × `t3.small.search`, 10 GB gp3, 0 replicas. With `-c production=true`: 3 × `m7g.medium.search` across 3 AZs, 1 replica, to be revisited against the real production domain before production is deployed. `t2.small` is not used because it doesn't support encryption at rest.
